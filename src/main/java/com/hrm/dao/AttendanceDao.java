...
public interface AttendanceDao {
	...
	@Select("""
			...
			""")
	List<AttendanceDto> searchAttendanceRecords(@Param("employeeId") String employeeId, @Param("name") String name,
			@Param("startDate") String startDate, @Param("endDate") String endDate, @Param("status") String status);

	// 특정한 근태 기록과 해당 사원 이름 조회
	@Select("SELECT a.*, e.name AS employeeName FROM Attendance a " + "JOIN Employee e ON a.EmployeeID = e.EmployeeID "
	...
	void updateAttendance(@Param("employeeId") int employeeId, @Param("date") LocalDate date,
			@Param("checkInTime") LocalTime checkInTime, @Param("checkOutTime") LocalTime checkOutTime,
			@Param("hoursWorked") Double hoursWorked, @Param("overtimeHours") Double overtimeHours,
			@Param("status") String status);

	
	@Select("SELECT a.*, e.name AS employeeName FROM Attendance a " + "JOIN Employee e ON a.EmployeeID = e.EmployeeID "
			+ "WHERE a.EmployeeID = #{employeeId} AND a.Date = #{date}")
	AttendanceDto getAttendanceByEmployeeAndDate(@Param("employeeId") Integer employeeId,
